...
async def compare_basket(body: CompareRequest, request: Request):
    """
    Compares a user's grocery_list across nearby stores (within radius_km)
    and returns totals per store + distance_km to each store.
    Response shape:
      {
        "results": [
    ...
    try:
      ...
      async with request.app.state.db.acquire() as conn:
          ...
          store_rows = await conn.fetch(
              "SELECT id, name, lat, lon FROM stores"
          )
          ...
          for sid, total in totals_by_id.items():
              ...
              dist = store_dist_km.get(sid)  # should exist because filtered by radius
