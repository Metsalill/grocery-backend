...
Backfill phase:
  ...
  After backfilling, drop to ~200 per run (set MIRROR_LIMIT=200 or pass --limit 200).
        ...
        while remaining > 0:
            ...
            for r in rows:
                ...
                try:
                    ...
                    # Skip SVGs & obvious non-raster types
                    if "svg" in ctype or ctype.startswith("text/"):
                        print(f"[{pid}] skipping unsupported type: {ctype}")
                        skipped += 1
                        continue
