    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for base in CONSUMER_API_BASES:
                ...
                resp = context.request.get(api_url, params={"language": language}, timeout=30000)
                if resp.ok:
                    try:
                        return resp.json()
                    except Exception:
    ...
    def walk(obj: Any):
        if isinstance(obj, dict):
            for k, v in obj.items():
                if (
                    k == "items" and isinstance(v, list) and v
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for it in items:
                ...
                price_eur = raw_price if isinstance(raw_price, float) and raw_price < 1000 else _cents_to_eur(raw_price)
                ...
                norm_items.append({
                    ...
                    "unit_price": {"price": unit_price_value_eur, "unit": up.get("unit") if isinstance(up, dict) else ""},
            ...
            return {"category": {"name": cat_name, "id": category_slug}, "items": norm_items, "source_url": cat_url}
        except Exception:
            return None
        finally:
            context.close()
...
def fetch_category_json(
    ...
    if data is not None:
        ...
        return data
    data = playwright_fetch_consumer_api(store_url, venue_slug, category_slug, language, headers)
    if data is not None:
        data["source_url"] = (
            f"{CONSUMER_API_BASES[0]}/venues/slug/{venue_slug}/assortment/"
            f"categories/slug/{category_slug}?language={language}"
...
def extract_rows(
    ...
    for it in items:
        ...
        price_eur = raw_price if isinstance(raw_price, float) and raw_price < 1000 else _cents_to_eur(raw_price)
        unit_price_value_eur = (
            unit_price_obj.get("price") if isinstance(unit_price_obj.get("price"), float) and unit_price_obj.get("price") < 1000
            else (_cents_to_eur(unit_price_obj.get("price")) if isinstance(unit_price_obj, dict) else None)
        )
            ...
            for r in rows:
                ...
                try:
                    cur.execute(
                        ...
                        """,
                        (
                            "wolt", r.get("ext_id") or "", r.get("name") or "",
...
def main():
    ...
    for idx, slug in enumerate(slugs, start=1):
        ...
        if not args.skip_ingest:
            for r in rows:
                all_rows_for_db.append(dict(
                    chain="Coop", channel="wolt",
                    store_name=store_host.split(":", 1)[-1].replace("-", " "),
                    store_host=store_host, city_path=city,
                    category_name=r.get("category_name") or slug,
                    ext_id=r.get("_ext_id"), name=r.get("name"),
                    brand=None, manufacturer=None, size_text=None,
                    price=r.get("price") if isinstance(r.get("price"), (int, float)) else None,
                    currency="EUR", image_url=r.get("image_url"),
                    url=src_url or store_url, description=r.get("description"),
                    ean_raw=r.get("barcode_gtin"), scraped_at=scraped_at,
                ))

        print(f"[ok] {idx:>3}/{len(slugs)}  '{slug}' → {len(rows)} item(s)")
        time.sleep(0.12)

    # ── FIX: use store_host for filename instead of venue_id ──
    store_slug = store_host.split(":", 1)[-1]  # e.g. "konsum-saku"
    out_path = (
        Path(args.out)
        if args.out
        else Path(args.out_dir) / f"coop_wolt_{store_slug}_{_normalize_city(city)}.csv"
    )
