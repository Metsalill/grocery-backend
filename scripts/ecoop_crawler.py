...
2) Visits each PDP and extracts: name, brand/manufacturer, size_text, price, EAN/GTIN, image, URL.
3) Streams rows to CSV during the crawl.
4) Bulk-upserts rows into your canonical DB via upsert_product_and_price(...).

Backwards-compat flags supported (ignored when not useful):
  --store-id <int>            : preferred over env STORE_ID; otherwise host → id map is used
  --store-host <host>         : override inferred host (e.g. coophaapsalu.ee)
  --store-url <url>           : alias for --region
  --upsert-db <name>          : accepted (ignored); DATABASE_URL env is used instead
"""
...
def _install_quiet_io():
    try:
        sys.stdout = _SafeWriter(sys.stdout)  # type: ignore
        sys.stderr = _SafeWriter(sys.stderr)  # type: ignore
    except Exception:
        pass
...
def normalize_ean(e: Optional[str]) -> Optional[str]:
    ...
    if len(d) in (8, 12, 13, 14):
        if len(d) == 14 and d.startswith("0"):
            d = d[1:]
        if len(d) == 12:  # UPC-A → pseudo EAN-13
            d = "0" + d
        return d
    return None
...
async def collect_category_product_links(page: Any, category_url: str, page_limit: int, req_delay: float, max_depth: int = 2) -> List[str]:
    ...
    while queue and not STOP_REQUESTED:
        ...
        # pagination
        try:
            next_links = await page.eval_on_selector_all('a[rel="next"], a[href*="?page="]', 'els => els.map(e => e.getAttribute("href"))')
        except Exception:
            next_links = []
        ...
        if depth < 2 and not STOP_REQUESTED:
            try:
                subcats = await page.eval_on_selector_all('a[href*="/tootekategooria/"]', "els => els.map(e => e.getAttribute('href'))")
            except Exception:
                subcats = []
            for sc in subcats:
                sc = norm_abs(sc or "")
...
async def extract_visible_price(page: Any) -> Optional[float]:
    ...
    () => {
      const sel = [
        '[data-testid="product-price"]','[data-test="product-price"]','.product-price','.price','.current-price','[class*="price"]'
      ];
      const out = [];
      const reSpace = /[\\s\\u00A0\\u2009\\u202F]+/g;
      for (const s of sel) {
...
async def extract_pdp(
    ...
    async def safe_goto(target_url: str) -> str:
        ...
        if goto_strategy in ("domcontentloaded", "networkidle", "load"):
            ...
            order = ["domcontentloaded", "load", "networkidle"]
        last_err = None
        for ws in order:
            try:
...
async def _bulk_ingest_to_db(rows: List[Tuple[Any, ...]], store_id: int) -> None:
    if store_id <= 0:
...
async def run_ecoop(args, categories: List[str], base_region: str, on_rows) -> None:
    ...
    # allow override via --store-host
    store_host = args.store_host.strip().lower() or map_store_host(base_region)

    pw_cm = async_playwright()
    pw = await pw_cm.start()
    browser = await pw.chromium.launch(headless=_to_bool(args.headless))
        ...
        for cat in categories:
            ...
            try:
                links = await collect_category_product_links(
                    page, cat, args.page_limit, args.req_delay, max_depth=2
                )
            finally:
            ...
            try:
                for coro in asyncio.as_completed(tasks):
                    ...
                    if r:
                        pending.append(r)
                        if len(pending) >= 25:
                            on_rows(pending); pending = []
            finally:
                still = [t for t in tasks if not t.done()]
                for t in still: t.cancel()
                if still: await asyncio.gather(*still, return_exceptions=True)
...
async def main(args):
    ...
    if args.categories_file and Path(args.categories_file).exists():
        categories.extend([
            ln.strip() for ln in Path(args.categories_file).read_text(encoding="utf-8").splitlines() if ln.strip()
        ])
    if not categories:
        print("[error] No category URLs provided. Pass --categories-multiline or --categories-file.")
        sys.exit(2)

    # region (allow --store-url alias)
    region_arg = args.region or args.store_url
    base_region = _normalize_region(region_arg, categories)
    canonical_host = map_store_host(base_region)
    if args.store_host:
        canonical_host = args.store_host.strip().lower()
    # store_id selection: CLI → env → host map
    try:
        cli_store_id = int(args.store_id) if args.store_id is not None else 0
    except Exception:
        cli_store_id = 0
    try:
        ...
        env_store_id = 0
    store_id = cli_store_id or env_store_id or map_store_id(canonical_host)
    print(f"[coop-ecoop] using store_id={store_id} (host={canonical_host})")
    ...
    if presharded and args.cat_shards > 1:
        print("[shard] Detected pre-sharded input → ignoring internal sharding.")
        cat_shards = 1; cat_index = 0

    if cat_shards > 1:
        if cat_index < 0 or cat_index >= cat_shards:
            print(f"[error] --cat-index must be in [0, {cat_shards-1}]"); sys.exit(2)
    ...
    if args.rotate_buckets and args.rotate_buckets > 1:
        idx = (dt.datetime.utcnow().hour % args.rotate_buckets) if args.rotate_index < 0 else (args.rotate_index % args.rotate_buckets)
        kept = [u for u in categories if _stable_bucket(u, args.rotate_buckets, args.rotate_salt or "") == idx]
        print(f"[rotate] buckets={args.rotate_buckets} index={idx} salt={args.rotate_salt!r} → keeping {len(kept)}/{len(categories)} URLs")
        categories = kept
        if not categories:
    ...
    # ingest
    await _bulk_ingest_to_db(rows_for_ingest, store_id)
...
def parse_args():
    ...
    p.add_argument("--store-url", dest="store_url", default="",
                   help="Alias for --region (kept for workflow compatibility)")
    p.add_argument("--store-host", dest="store_host", default="",
                   help="Override inferred store host (e.g. coophaapsalu.ee)")
    p.add_argument("--store-id", dest="store_id", default=0, type=int,
                   help="Override store id; falls back to env STORE_ID then host→id map")
    p.add_argument("--upsert-db", dest="upsert_db", default="", help="Accepted but unused; DATABASE_URL env is used")
    ...
    # crawl limits
    p.add_argument("--page-limit", type=int, default=0, help="Max product links per category (0=all)")
    p.add_argument("--max-products", type=int, default=0, help="Cap per category after discovery (0=all)")

    # concurrency & behavior
    p.add_argument("--headless", default="1", help="1/0/true/false for headless browser")
    ...
    p.add_argument("--rotate-buckets", type=int, default=1, help="Stable buckets by URL hash (1=disable)")
    p.add_argument("--rotate-index", type=int, default=-1, help="Bucket to crawl (0..N-1). -1 = auto by UTC hour.")
    p.add_argument("--rotate-salt", type=str, default="", help="Optional salt for bucket assignment")

    # output
    p.add_argument("--out", default="out/coop_ecoop.csv", help="CSV file or output directory")
    p.add_argument("--write-empty-csv", action="store_true", default=True, help="Write header even if no rows")

    # robustness
    p.add_argument("--goto-strategy", choices=["auto","domcontentloaded","networkidle","load"], default="auto",
                   help="Playwright wait_until strategy for PDP navigation.")
    p.add_argument("--nav-timeout", type=int, default=45000, help="Navigation timeout (ms) for PDP pages.")
    return p.parse_args()
