...
async def _resolve_product_ids_by_name(pool, names: List[str]) -> Dict[str, int]:
    """
    Resolve product names to IDs via `products` table (schema has `name`, not `product`).
    """
    q = """
...
async def compare_basket_service(
    ...
    # ---- Path A: IDs via products.name + view v_latest_store_prices
    try:
        name_to_id = await _resolve_product_ids_by_name(pool, names_lower)
    except (pgerr.UndefinedTableError, pgerr.UndefinedObjectError, pgerr.UndefinedColumnError) as e:
    ...
    if not use_fallback_name_mode:
        ...
        sql_geo = """
        WITH candidates AS (
          SELECT lsp.store_id, lsp.product_id, lsp.price::numeric AS price
        ...
        SELECT ps.store_id, s.name AS store_name,
        ...
        if not use_fallback_name_mode:
            ...
            if rows:
                best_store_id = int(rows[0]["store_id"])
                async with pool.acquire() as conn:
                        ...
                        items_array = [
                            {
                                ...
                                "line_total": round((price_by_pid.get(pid, 0.0) * qty_map.get(pid, 1)), 2),
                            }
    ...
    # ---- Path B: NAME-based (join prices ↔ products, filter by products.name)
    qty_array = [qty_by_lower[n] for n in names_lower]

    sql_latest_by_name = """
    WITH latest AS (
      SELECT DISTINCT ON (p.store_id, p.product_id)
        p.store_id,
        p.product_id,
        p.price::numeric AS price,
        p.seen_at
      FROM prices p
      ORDER BY p.store_id, p.product_id, p.seen_at DESC
    ),
    candidates AS (
      SELECT l.store_id, l.product_id, l.price
      FROM latest l
      JOIN products pr ON pr.id = l.product_id
      WHERE LOWER(pr.name) = ANY($1::text[])
    )
    SELECT
      c.store_id,
    ...
    JOIN (
      ...
      SELECT LOWER(pr2.name) FROM products pr2 WHERE pr2.id = c.product_id
    )
            ...
            try:
                rows = await conn.fetch(
                    sql_latest_by_name.replace(
                        "GROUP BY c.store_id, s.name",
                        """AND earth_distance(ll_to_earth($3::float8, $4::float8), ll_to_earth(s.lat, s.lon)) <= $5::int
                           GROUP BY c.store_id, s.name"""
                    ),
    ...
    for r in rows:
        stores_payload.append({
            ...
            "distance_km": None,  # not computing distance here; optional add later
            "items": [],
        })
